...
`CCCD` | 説明
:---:|---
`0000` | デジタルデコーダーリセット - A Digital Decoder Reset shall erase all volatile memory 115 (including and speed and direction data), and return to its initial power up state as defined in S9.2.4 section A. Command Stations shall not send packets to addresses 112-127 for 10 packet times following a Digital Decoder Reset. This is to ensure that the decoder does not start executing service mode instruction packets as operations mode packets (Service Mode instruction packets have a short address in the range of 112 to 127 decimal.)
`0001` | 強制リセット - Configuration Variables 29, 31 and 32 are reset to its factory default conditions, CV#19 is set to "00000000" and a Digital Decoder reset (as in the above instruction) shall be performed.
`1111` | デコーダー応答要求

`CCC` | 説明
:---:|---
`001` | 工場試験命令 - This instruction is used by manufacturers to test decoders at the factory. It must not be sent by any command station during normal operation. This instruction may be a multi-byte instruction.
`010` | 将来の使用のため予約
`011` | デコーダーフラグの設定（下記参照）
`100` | 将来の使用のため予約
`101` | 発展アドレスの設定 (CV#29 bit 5)
`110` | 将来の使用のため予約
