# S-9.2.1 デジタルコマンドコントロールの拡張パケット形式 {#extended-packet-formats-for-digital-command-control}
...
NMRA の「デジタルコマンドコントロールの通信標準（S-9.2）」は相互運用性のための最低限で基本的なパケット形式を提供しています。この標準は、発生する現実的な運用を可能にする枠組みを提供する拡張パケット形式を提供します。この形式は S-9.2 で定義される一般パケット形式にしたがいます。基本パケットは `0` ビットで分割された3バイトでしたが、拡張パケット形式では `0` ビットで分割された3から6バイトをもちえます。

### 形式定義 {#format-definition}
...
## B：多機能デジタルデコーダー用ブロードキャストコマンド {#broadcast-command-for-multi-function-digital-decoders}
...
## C：多機能デジタルデコーダー用命令パケット {#instruction-packets-for-multi-function-digital-decoders}
...
### デコーダー制御命令と編成制御命令（`000`） {#decoder-and-consist-control-instruction}

デコーダー応答機能（`00001111`）をのぞいて、単一のデコーダーと編成制御命令のみひとつのパケットにふくめることができます。

#### デコーダー制御（`0000`） {#decoder-control}
...
`011D` | デコーダーフラグの設定
`100D` | 将来の使用のため予約
`101D` | 拡張アドレスの設定（CV 29 のビット5）
`110D` | 将来の使用のため予約
`111D` | デコーダー応答要求

##### デジタルデコーダーリセット {#digital-decoder-reset}
...
命令 `100` と `101` によって制御される機能はデコーダー基本アドレスに返答しつづけます。命令 `100` と `101` によって制御される機能は、CV 21 と 22 の適切なビットが有効ならば編成アドレスにも応答します。
...
`0011` | 2番目のバイトで指定された編成アドレスを設定し、編成を有効化します。編成アドレスは CV 19 のビット0～6に保存され、CV 19 のビット7は `1` の値に設定されます。編成内のこの車両の方向は通常と逆の方向です。編成アドレスが `0000000` なら編成は無効化されます。
その他 | 将来の使用のために予約

### 発展的制御命令（`001`） {#advanced-operations-instruction}
...
#### 128段階速度制御 {#128-speed-step-control}
...
#### 制限された速度制御命令 {#restricted-speed-step-instruction}
...
### 速度方向命令（`010`・`011`）{#speed-and-direction-instructions}
...
### 第1機能群命令（`100`）[^4] {#function-group-one-instruction}
...
### 第2機能群命令（`101`）[^5] {#function-group-two-instruction}
...
### 機能拡張命令（`110`） {#feature-expansion-instruction}
...
#### 2値状態制御命令（長形式） {#binary-state-control-instruction-long-form}
...
#### 2値状態制御命令（短形式） {#binary-state-control-instruction-short-form}
...
#### F13～F20 機能制御 {#f13-f20-function-control}
...
#### F21～F28 機能制御 {#f21-f28-function-control}
...
### CV 読み書き命令（`111`） {#configuration-variable-access-instruction}
...
#### CV 読み書き応答 {#configuration-variable-access-acknowledgment}

CV 読み書き応答が要求され、運転モード応答が有効であるとき、デコーダーは運転モード応答によって応答しなければいけません。

#### CV 読み書き手順（短式） {#configuration-variable-access-instruction-short-form}
...
#### CV 読み書き手順（長式） {#configuration-variable-access-instruction-long-form}
...
## D：アクセサリーデジタルデコーダーパケット形式 {#accessory-digital-decoder-packet-formats}

アクセサリーデジタルデコーダーは、転轍機制御やライトのオン・オフなどの複数の単純な機能の制御に使用されます。複数のアドレスに応答するデジタルデコーダーを開発して、単一のデジタルデコーダーでより多くのデバイスを制御することもできます。

### 基本アクセサリーデコーダーパケット形式 {#basic-accessory-decoder-packet-format}
...
### 拡張アクセサリーデコーダー制御パケット形式 {#extended-accessory-decoder-control-packet-format}
...
### アクセサリーデコーダー用ブロードキャストコマンド {#broadcast-command-for-accessory-decoders}

#### 基本アクセサリーデコーダー用ブロードキャストコマンド {#broadcast-command-for-basic-accessory-decoders}
...
#### 拡張アクセサリーデコーダー用ブロードキャストコマンド {#broadcast-command-for-extended-accessory-decoders}
...
### アクセサリーデコーダー CV 読み書き手順 {#accessory-decoder-configuration-variable-access-instruction}
...
#### 運転モードプログラミングのための基本アクセサリーデコーダーパケットアドレス {#basic-accessory-decoder-packet-address-for-operations-mode-programming}
...
#### 運転モードプログラミングのための拡張デコーダー制御パケットアドレス {#extended-decoder-control-packet-address-for-operations-mode-programming}
...
## E：運転モード応答 {#operations-mode-acknowledgment}
...
## アクセサリーデコーダー CV 読み書き手順[^9] {#accessory-decoder-configuration-variable-access-instruction}
