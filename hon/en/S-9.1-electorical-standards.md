...
duration within the t1 range. The duration difference between the first and last parts of a `1` bit shall not exceed t1d. A Digital Decoder must accept bits whose first and last parts have a duration within the $$t_{r1}$$ range as a valid bit with the value of `1`. Digital Decoders must accept `1` bits where the duration difference between the positive and negative components does not exceed $$t_{r1d}$$.
